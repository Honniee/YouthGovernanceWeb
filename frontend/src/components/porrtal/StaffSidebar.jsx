...
const StaffSidebar = ({ sidebarOpen, setSidebarOpen, onLogout }) => {
  ...
  const navigation = [
    ...
    { 
      name: 'SK Terms', 
      href: '/staff/sk-governance/terms', 
      icon: Calendar,
    ...
    { 
      name: 'Term History', 
      href: '/staff/sk-governance/term-history', 
      icon: History,
  ...
  const isActive = (href) => {
    // Special case for dashboard - check if we're on staff root or dashboard
    if (href === '/staff/dashboard') {
      return location.pathname === '/staff' || location.pathname === '/staff/' || location.pathname === '/staff/dashboard';
    }
      ...
      >
        <div className="overflow-y-auto py-4 px-3 h-full bg-gradient-to-b from-white to-gray-50/30 dark:from-gray-800 dark:to-gray-900/30 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-gray-600 dark:scrollbar-track-gray-800">
          ...
          <ul className="space-y-2">
            {orderedCategories
              .filter(category => groupedNavigation[category])
              .map(category => [category, groupedNavigation[category]])
              .map(([category, items]) => {
