    ...
    <PublicLayout>
      ...
      <section className="pt-8 pb-16 md:py-16 bg-white relative overflow-hidden">
        ...
        >
          {/* Section header */}
          <div className="mb-12">
            <div className="inline-flex items-center px-3 py-1 rounded-full bg-[#E7EBFF] text-[#24345A] text-xs font-semibold uppercase tracking-wider mb-4">Recent</div>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Recent Updates</h2>
            <p className="text-gray-600 max-w-3xl text-lg">The latest five items across programs, projects, activities, and announcements.</p>
          </div>
                      ...
                      >
                        ...
                        >
                          ...
                          <div className={`relative rounded-2xl overflow-hidden transition-[height] duration-500 ease-in-out ${isActive ? 'h-[22rem] sm:h-[24rem] md:h-[28rem]' : 'h-64 sm:h-72 md:h-80 lg:h-[22rem]'}`}>
                            ...
                            <img
                              src={card.image || getFallbackImage(card.category, card.title)}
                              alt={card.title}
                              className="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                              loading="lazy"
                              decoding="async"
                              onError={(e) => { e.currentTarget.src = getFallbackImage(card.category, card.title); }}
                            />
                            {/* Gradient overlay */}
              ...
              <div className="relative inline-grid grid-cols-7 items-center rounded-full bg-gray-100 ring-1 ring-gray-300 p-2 shadow-sm overflow-hidden">
                ...
                {categories.map((category) => (
                  <button
                    ...
                    className={`relative z-10 px-4 py-3 text-sm rounded-full transition-colors duration-300 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#24345A]/30 ${
