  ...
  return (
    <PublicLayout>
      <section className="relative overflow-hidden bg-gradient-to-br from-[#24345A] via-[#1a2a47] to-[#0f1a2e] text-white -mt-12 sm:mt-0">
        ...
        <div ref={heroRef} className={`relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 transition-all duration-1000 ease-out ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-2">Featured Programs</h1>
            <p className="text-white/90 text-lg md:text-xl mb-4">All published announcements categorized as programs.</p>
            <p className="text-white/80">Total programs: {programsTotal.toLocaleString()}</p>
          </div>
        </div>
      </section>
        ...
        <div ref={listRef} className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-1000 ease-out ${listVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          {loading && (
            <div className="flex items-center justify-center py-16">
              <LoadingSpinner variant="spinner" message="Loading featured programs..." size="md" color="blue" height="h-32" />
            </div>
          )}
          {error && !loading && (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Failed to Load Featured Programs</h3>
                  ...
                  >
                    <div className="relative overflow-hidden aspect-[16/9]">
                      ...
                      <div className="absolute top-2 right-2">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center bg-yellow-400">
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      ...
                      {announcement.summary && (<p className="text-gray-600 text-sm mb-4 leading-relaxed line-clamp-3 flex-1">{announcement.summary}</p>)}
                      {(announcement.event_date || announcement.location || announcement.end_date) && (
                        <div className="space-y-2 mb-4 p-3 bg-gray-50 rounded-lg">
                          {(() => {
                            ...
                            if (hasEvent && hasEnd) return (<div className="flex items-center text-sm text-gray-600"><Calendar className="w-4 h-4 mr-2 text-blue-500" /><span>Event period: {formatDate(announcement.event_date)} – {formatDate(announcement.end_date)}</span></div>);
                            if (hasEvent) return (<div className="flex items-center text-sm text-gray-600"><Calendar className="w-4 h-4 mr-2 text-blue-500" /><span>Event date: {formatDate(announcement.event_date)}</span></div>);
                            if (hasEnd) return (<div className="flex items-center text-sm text-red-600"><Clock className="w-4 h-4 mr-2" /><span>Deadline: {formatDate(announcement.end_date)}</span></div>);
                            return null;
                          })()}
                          {announcement.location && (<div className="flex items-center text-sm text-gray-600"><MapPin className="w-4 h-4 mr-2 text-gray-600" /><span className="truncate">{announcement.location}</span></div>)}
                        </div>
                      )}
