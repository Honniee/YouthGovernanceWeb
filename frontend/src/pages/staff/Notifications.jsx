...
const Notifications = () => {
  ...
  // Map notification type to badge name
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (backendTotal && mappedNotifications.length !== backendTotal) {
          logger.warn('Count mismatch detected', {
        ...
        logger.error('Invalid API response format', null, { response });
  ...
  const notificationStats = useMemo(() => {
    ...
    logger.debug('Notification Stats', {
      total,
      unread,
      highPriority,
      today,
...
${bodyRows}
      ...
      <html>
        ...
        <body>
          <div class="instructions">
            <div><b>NOTIFICATIONS REPORT</b></div>
  ...
  const mainExport = useExport({
    exportFunction: async (format, style = null) => {
      try {
        ...
        if (format === 'csv') {
          ...
          openNotificationPrintPdf('System Notifications', filteredNotifications);
        } else if (format === 'xlsx' || format === 'excel') {
          const xml = buildNotificationsExcelXml(filteredNotifications);
          downloadExcel('notifications.xls', xml);
        }
    ...
    onSuccess: () => showSuccessToast('Export completed', 'Notifications exported successfully'),
  ...
  const confirmDeleteNotification = async () => {
    ...
    try {
      ...
      logger.info('Notification deleted successfully', { notificationId: notificationToDelete.id });
    } catch (error) {
  ...
  const handleBulkMarkAsRead = async () => {
    logger.debug('handleBulkMarkAsRead called', { selectedCount: selectedNotifications.length });
    ...
    logger.debug('Unread selected notifications', { unreadCount: unreadSelected.length });
    ...
    try {
      ...
      logger.error('Failed to mark notifications as read', error, { count: unreadSelected.length });
  ...
  const handleBulkDelete = async () => {
    logger.debug('handleBulkDelete called', { selectedCount: selectedNotifications.length });
  ...
  const confirmBulkDelete = async () => {
    logger.debug('Bulk delete confirmation confirmed');
    ...
    try {
      ...
      logger.error('Bulk delete error', error, { count: selectedNotifications.length });
  ...
  return (
    <div className="space-y-6">
      ...
      <HeaderMainContent 
        title="Notifications" 
        description="Stay updated with system alerts, user activities, and important announcements."
      />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            <button
              ...
              className="inline-flex items-center justify-center px-3 py-2 border border-blue-200 text-blue-700 text-sm font-medium rounded-lg hover:bg-blue-100 transition-colors disabled:opacity-50"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50"
            >
              {markingAllAsRead ? (
                <>
                ...
                <div className="flex items-center gap-3 overflow-x-auto pb-2 md:pb-0">
                  <div className="flex items-center space-x-3 min-w-max">
                    ...
                    <button 
                      ...
                      className={`inline-flex items-center border rounded-lg font-medium transition-all duration-200 px-2 py-1.5 sm:px-3 sm:py-2 whitespace-nowrap flex-shrink-0 ${
                        !(sortBy === 'createdAt' && sortOrder === 'desc') ? 'border-blue-500 text-blue-600 bg-blue-50' : 'border-gray-200 text-gray-700 hover:border-gray-300 hover:bg-gray-50'
                      }`}
                    >
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {paginatedNotifications.length === 0 ? (
                      ...
                      paginatedNotifications.map((notification) => (
                        <tr 
                          key={notification.id} 
                          className={`hover:bg-gray-50 transition-colors cursor-pointer ${!notification.isRead ? 'bg-blue-50/30' : ''}`}
                          onClick={(e) => {
            ...
            <div className="p-6 sm:p-8">
               ...
               <div className="flex items-start sm:items-center justify-between mb-6 gap-4">
                 <div className="flex items-center space-x-4 min-w-0 flex-1">
                   <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center flex-shrink-0 shadow-sm border border-blue-200/50">
                   ...
                   <div className="flex flex-col sm:flex-row gap-3">
                     {!selectedNotification.isRead && (
                       <button
                         ...
                         className="inline-flex items-center justify-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-xl text-blue-700 bg-blue-100 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 shadow-sm hover:shadow-md"
                       >
                         <Check className="w-4 h-4 mr-2" />
                         Mark as Read
                       </button>
