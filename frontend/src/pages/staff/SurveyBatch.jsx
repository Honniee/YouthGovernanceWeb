...
const SurveyBatch = () => {
  ...
  const loadBatchData = async (...args) => {
    ...
    try {
      ...
      logger.debug('Survey batches API response', { success: response.success, hasData: !!response.data });
      
      if (response.success) {
        const batchData = response.data.data || [];
        logger.debug('Survey batches loaded', { count: batchData.length, firstBatch: batchData[0]?.batchId || batchData[0]?.id });
  ...
  const loadBatchStats = useCallback(async (opts = { silent: false }) => {
    try {
      ...
      if (response.success) {
        ...
        logger.debug('Backend batch stats data', backendData);
        ...
        logger.debug('Mapped batch stats', mappedStats);
