...
import StaffDashboardSurveyAnalytics from './StaffDasboardSurveyAnalytics';
...
const StaffDashboard = () => {
  ...
  const [stats, setStats] = useState({
    ...
    // SK Governance (read-only for staff)
    sk: {
      activeOfficials: 0,
      totalOfficials: 0
    },
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch staff-relevant data in parallel
      const [
        validationStatsResp,
        surveyStatsResp,
        recentActivityResp,
        ...
        api.get('/sk-officials/statistics').catch(() => ({ success: false, data: {} }))
      ]);
      ...
      if (validationStatsResp?.data?.success && validationStatsResp?.data?.data) {
        ...
        setStats(prev => ({
          ...
          tasks: {
            ...
            pending: vqStats.pending || 0,
            completed: vqStats.completed || 0,
            completionRate: (vqStats.pending + vqStats.completed) > 0 
              ? Math.round((vqStats.completed / (vqStats.pending + vqStats.completed)) * 100)
              : 0
          },
          youth: {
            ...prev.youth,
      ...
      if (surveyStatsResp?.success && surveyStatsResp?.data) {
        ...
        setStats(prev => ({
          ...prev,
          surveys: {
            active: surveyData.activeBatches || 0,
            total: surveyData.totalResponses || 0,
            pendingValidation: validationStatsResp?.data?.data?.pending || 0,
            completed: surveyData.completedResponses || 0,
            completionRate: surveyData.completionRate || 0,
            avgResponseTime: surveyData.avgResponseTime || 0
          }
        }));
      }
      ...
      if (skStatsResp?.data?.success && skStatsResp?.data?.data) {
        const skData = skStatsResp.data.data;
        setStats(prev => ({
          ...prev,
          sk: {
            activeOfficials: skData.active || 0,
            totalOfficials: skData.total || 0
          }
        }));
      }
  ...
  const StatCard = ({ title, value, icon: Icon, color, trend, subtitle, onClick, analytics, insights }) => {
    ...
    const badgeText = subtitle || (trend ? `${trend > 0 ? '+' : ''}${trend}%` : '');
  ...
  const AlertCard = ({ alert }) => (
    <div className={`alert-card p-4 ${
      ...
      <div className="flex items-center justify-between">
        ...
        {alert.action && (
                      <Button size="sm" onClick={() => window.location.href = '/staff/validation-queue'}>
              {alert.action}
            </Button>
        )}
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex-1">
          <div className="relative">
            <input
              type="text"
              placeholder="Search activities, surveys, or tasks..."
              value={searchQuery}
        ...
        <div className="flex flex-col items-center justify-center min-h-[600px]">
          <div className="text-center space-y-4">
            ...
            <div className="flex flex-col items-center space-y-1">
              ...
              <p className="text-xs text-gray-500">Fetching your data and statistics</p>
            </div>
          </div>
      </div>
    );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={stats.tasks.pending.toLocaleString()}
          icon={Clock}
          color="yellow"
          subtitle="Requires Review"
          insights="Pending validation tasks"
          onClick={() => window.location.href = '/staff/validation-queue'}
        />
        <StatCard
          title="Completed Tasks"
          value={stats.tasks.completed.toLocaleString()}
          icon={CheckCircle}
          color="green"
          subtitle={`${stats.tasks.completionRate}% Rate`}
          insights="Completed validation tasks"
          onClick={() => window.location.href = '/staff/validation-queue'}
        />
        <StatCard
          title="Active Surveys"
          value={stats.surveys.active.toLocaleString()}
          icon={FileText}
          color="blue"
          subtitle="Active Batches"
          insights="Currently active survey batches"
          onClick={() => window.location.href = '/staff/survey/batches'}
        />
        <StatCard
          title="Total Responses"
          value={stats.surveys.total.toLocaleString()}
          icon={FileText}
          color="teal"
          subtitle={`${stats.surveys.completionRate}% Complete`}
          insights="Total survey responses"
          onClick={() => window.location.href = '/staff/survey/batches'}
        />
        <StatCard
          ...
          icon={Calendar}
          color="indigo"
          subtitle={hasActiveTerm && activeTerm ? "Active Term" : 'No active term'}
          ...
          onClick={() => window.location.href = '/staff/sk-governance'}
        />
        <StatCard
          title="SK Officials"
          value={stats.sk.activeOfficials.toLocaleString()}
          icon={Users}
          color="green"
          subtitle="Active Officials"
          insights="Active SK officials"
          onClick={() => window.location.href = '/staff/sk-officials'}
        />
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-green-50 to-emerald-50">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => window.location.href = '/admin/survey/batches'}
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <QuickAction
              ...
              icon={Eye}
              onClick={() => window.location.href = '/admin/survey/validation'}
              color="yellow"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          ...
          onClick={() => window.location.href = '/admin/users/youth?filter=age_warning'}
        />
        <StatCard
          title="Pending Validations"
        ...
        <StatCard
          ...
          onClick={() => window.location.href = '/admin/system/activity-logs'}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <StatCard
            title="Published"
            value={stats.system.published}
            icon={CheckCircle}
            color="green"
            subtitle="Public"
            onClick={() => window.location.href = '/admin/announcements?filter=published'}
          />
          <StatCard
            title="Drafts"
            value={stats.system.drafts}
            icon={Edit}
            color="orange"
            subtitle="Pending"
            onClick={() => window.location.href = '/admin/announcements?filter=drafts'}
          />
        </div>
      </div>
    </div>
  ...
  return (
    ...
    <div className="space-y-6">
      <HeaderMainContent
        ...
        description="Overview of your tasks, surveys, and activities"
      >
