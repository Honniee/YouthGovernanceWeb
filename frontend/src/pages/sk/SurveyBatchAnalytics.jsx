 ...
 * Recharts-based analytics visuals similar to your inspo screenshots.
 * Props:
 *  - responses: array of response objects from batch
 */
const SurveyBatchAnalytics = ({ responses = [], startDate, endDate }) => {
  const [locationFilter, setLocationFilter] = useState('all');
  const [modal, setModal] = useState({ open: false, section: null });

  const locations = useMemo(() => {
    return Array.from(new Set((responses || []).map(r => r.location || r.barangay).filter(Boolean)));
  }, [responses]);

  const filtered = useMemo(() => {
    if (locationFilter === 'all') return responses || [];
    return (responses || []).filter(r => (r.location || r.barangay) === locationFilter);
  ...
  const totals = useMemo(() => {
    ...
    const validated = filtered.filter(r => (r.status || r.validationStatus || '').toString().toLowerCase() === 'validated').length;
    ...
    const registered = filtered.filter(r => !!(r.registeredVoter ?? r.isRegisteredVoter)).length;
    const attended = filtered.filter(r => !!(r.attendedKKAssembly ?? r.attended_KK_assembly)).length;
    ...
    const ages = (filtered || []).map(r => {
      ...
      const b = calcAgeFromBirthday(r.birthday ?? r.birthdate ?? r.dateOfBirth ?? r.dob);
      if (Number.isFinite(b)) return b;
      const g = parseAgeFromGroup(r.youthAgeGroup || r.ageGroup);
      return g;
  ...
  const classification = useMemo(() => countBy(r => r.youthClassification || r.classification), [filtered]);
  const civilStatus = useMemo(() => countBy(r => r.civilStatus), [filtered]);
  const ageGroups = useMemo(() => countBy(r => r.youthAgeGroup || r.ageGroup), [filtered]);
  const education = useMemo(() => countBy(r => r.education || r.highestEducationalAttainment).sort((a,b)=>b.value-a.value), [filtered]);
  const workStatus = useMemo(() => countBy(r => r.workStatus || r.employmentStatus), [filtered]);
  const barangays = useMemo(() => countBy(r => r.location || r.barangay).sort((a,b)=>b.value-a.value), [filtered]);
  ...
  const percentOnly = ({ percent }) => `${(percent * 100).toFixed(0)}%`;
  const formatDate = (d) => {
    if (!d) return null;
    const dt = new Date(d);
    ...
    <div className="space-y-8">
      ...
      {(startDate || endDate) && (
        <div className="flex items-center gap-3 text-sm text-gray-600">
          ...
          <span className="font-medium text-gray-900">{formatDate(startDate) || 'N/A'}</span>
          <span>–</span>
          <span className="font-medium text-gray-900">{formatDate(endDate) || 'N/A'}</span>
        </div>
      )}
		...
		<div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden cursor-pointer hover:shadow-md transition" onClick={()=>setModal({ open: true, section: 'engagement' })}>
			...
			<div className="p-5">
				<ResponsiveContainer width="100%" height={chartHeight}>
					<BarChart data={[
						{ name: 'Registered Voter', Yes: filtered.filter(r => !!(r.registeredVoter ?? r.isRegisteredVoter)).length, No: totals.total - filtered.filter(r => !!(r.registeredVoter ?? r.isRegisteredVoter)).length },
						{ name: 'Voted Last Election', Yes: filtered.filter(r => !!(r.votedLastElection ?? r.voted_last_SK)).length, No: totals.total - filtered.filter(r => !!(r.votedLastElection ?? r.voted_last_SK)).length },
						{ name: 'Attended KK Assembly', Yes: filtered.filter(r => !!(r.attendedKKAssembly ?? r.attended_KK_assembly)).length, No: totals.total - filtered.filter(r => !!(r.attendedKKAssembly ?? r.attended_KK_assembly)).length }
					]}>
						<CartesianGrid strokeDasharray="3 3" />
...
export default SurveyBatchAnalytics;
