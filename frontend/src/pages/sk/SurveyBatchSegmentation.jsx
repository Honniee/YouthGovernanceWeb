...
const SurveyBatchSegmentation = ({ batchId, batchName }) => {
  ...
  // For SK users, force barangay scope and use their barangay
  const [selectedBarangay, setSelectedBarangay] = useState(skBarangay || 'all');
  const [scope, setScope] = useState('barangay'); // SK users can only see their barangay
  ...
  // Fetch segmentation data when batchId or barangay changes
  const fetchSegmentationData = useCallback(async () => {
    if (!batchId || !skBarangay) return;
    
    setLoading(true);
    setError(null);
    try {
      // For SK users, always filter by their barangay
      const effectiveBarangayId = skBarangay;
      // Fetch segments, stats, and runs for this batch
      const filters = {
        scope: 'barangay', // SK users can only see barangay-level segmentation
  ...
  // Fetch segmentation data when batchId or barangay changes
  useEffect(() => {
    if (batchId && skBarangay) {
      fetchSegmentationData();
    }
  }, [batchId, skBarangay, fetchSegmentationData]);
  ...
  // Handle scope change - SK users can only use barangay scope
  const handleScopeChange = (newScope) => {
    // SK users are locked to barangay scope, so ignore scope changes
    if (newScope === 'municipality') {
      setSelectedBarangay(skBarangay || 'all');
    }
    // Always keep scope as 'barangay' for SK users
    setScope('barangay');
  };

  // Run clustering for this batch
  const handleRunClustering = async () => {
    const effectiveBarangayId = skBarangay || (selectedBarangay !== 'all' ? selectedBarangay : null);
    const barangayName = barangays.find(b => (b.id === effectiveBarangayId || b.barangay_id === effectiveBarangayId || b.name === effectiveBarangayId))?.name || effectiveBarangayId || 'your barangay';
    if (!confirm(`Run clustering for "${batchName}" (barangay: ${barangayName})?\n\nThis will create youth segments based on validated responses in this batch.\n\nEstimated time: 10-30 seconds`)) {
      return;
    }
    ...
    try {
      const response = await clusteringService.runClustering({
        scope: 'barangay', // SK users can only run barangay-level clustering
        barangayId: effectiveBarangayId,
        batchId: batchId
      });
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="px-5 py-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-wrap">
            {/* Scope Selector - Hidden for SK users (they can only see their barangay) */}
            {/* SK users are locked to barangay scope, so we don't show the scope selector */}
            
            {/* Current View Indicator */}
            <div className="text-sm text-blue-700 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-200">
              {skBarangay 
                ? `Viewing: ${barangays.find(b => (b.id === skBarangay || b.barangay_id === skBarangay || b.name === skBarangay))?.name || skBarangay || 'Your Barangay'}`
                : 'Viewing: Your Barangay'}
            </div>
          </div>
          ...
          <button
            ...
            disabled={runningClustering || !skBarangay}
            className={`inline-flex items-center px-4 py-2 rounded-lg font-medium transition-colors ${
              runningClustering || !skBarangay
