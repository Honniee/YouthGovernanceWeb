...
const SurveyBatchReport = () => {
      ...
      const loadBatch = async () => {
        try {
          ...
          if (batch) {
            ...
            // Extract batch name with multiple fallbacks
            const batchName = batch.batchName || batch.batch_name || batch.name || 'Survey Batch';
  ...
  // Load responses function - memoized with useCallback
  const loadResponses = useCallback(async (batchIdOverride = null) => {
  ...
  }, [reportBatch?.batchId, skBarangay]);
  ...
  }, [reportBatch?.batchId, viewMode, loadResponses]);

  // Update statistics from barangay-filtered responses (for SK users)
  useEffect(() => {
    if (skBarangay && reportBatch?.batchId) {
      // Calculate statistics from filtered responses
      const total = responses.length;
      const validated = responses.filter(r => (r.status || r.validationStatus || '').toString().toLowerCase() === 'validated').length;
      const pending = responses.filter(r => (r.status || r.validationStatus || '').toString().toLowerCase() === 'pending').length;
      const rejected = responses.filter(r => (r.status || r.validationStatus || '').toString().toLowerCase() === 'rejected').length;
      
      // Preserve original totalYouths from batch if available
      const originalTotalYouths = batchStatistics?.totalYouths || reportBatch?.statistics?.totalYouths || 0;
      ...
      const filteredStatistics = {
        ...
        totalYouths: originalTotalYouths, // Keep original target from batch
        totalYouthsSurveyed: total,
        totalYouthsNotSurveyed: Math.max(0, originalTotalYouths - total),
        responseRate: originalTotalYouths > 0 ? Math.round((total / originalTotalYouths) * 100) : 0,
      ...
      setBatchStatistics(filteredStatistics);
    }
  }, [responses, skBarangay, reportBatch?.batchId, reportBatch?.statistics]);
  ...
  // Helper function to anonymize youth names for privacy (same as display)
  const getAnonymousName = (response) => {
    const identifier = response.responseId || response.id;
    if (identifier) {
      return `Respondent #${identifier}`;
    }
    const stableKey = `${response.email || ''}_${response.location || response.barangay || ''}_${response.age || ''}`;
    const hash = stableKey.split('').reduce((acc, char) => {
      const hash = ((acc << 5) - acc) + char.charCodeAt(0);
      return hash & hash;
    }, 0);
    return `Respondent #${Math.abs(hash).toString().slice(0, 6)}`;
  };

  // Export functionality - Staff version: anonymized for privacy
  const buildBatchCsvRows = (responses = []) => {
    const rows = [];
    // Removed: Name, Email Address, Contact Number, Home Address for privacy
    rows.push([
      'Respondent ID', 'Region', 'Province', 'City/Municipality', 'Barangay', 'Age', 'Birthday M', 'Birthday D', 'Birthday Y',
      'Sex Assigned at Birth', 'Civil Status', 'Youth Classification', 'Age Group',
      'Highest Educational Attainment', 'Work Status', 'Registered Voter? (Y/N)',
    ...
    (responses || []).forEach((r) => {
      ...
      // Use anonymous identifier instead of actual name
      const anonymousId = getAnonymousName(r);
      const age = r.age ?? '';
      ...
      // Removed: email, phone, homeAddress for privacy
      ...
      rows.push([
        anonymousId, region, province, city, barangay, age, bMonth, bDay, bYear, sexAtBirth, civilStatus, classification,
        ageGroup, education, workStatus, regVoter, votedLast, attendedKK, attendanceCount
      ]);
    });
    return rows;
  };
  ...
  // Excel (XLS via XML) export for responses - Staff version: anonymized for privacy
  const buildResponsesExcelXml = (responses = []) => {
    const headerRow = `
      <Row>
        <Cell><Data ss:Type="String">Respondent ID</Data></Cell>
    ...
    const bodyRows = (responses || []).map((r) => {
      ...
      return `
      <Row>
        <Cell><Data ss:Type="String">${anonymousId}</Data></Cell>
        <Cell><Data ss:Type="String">${region}</Data></Cell>
        <Cell><Data ss:Type="String">${province}</Data></Cell>
...
${bodyRows}
  ...
  const openBatchPrintPdf = (title, responses = []) => {
    ...
    const rows = (responses || []).map((r) => {
      ...
      return `
        <tr>
          <td class="respondent-id">${anonymousId || '&nbsp;'}</td>
          <td>${region || '&nbsp;'}</td>
          <td>${province || '&nbsp;'}</td>
          <td>${city || '&nbsp;'}</td>
          <td>${barangay || '&nbsp;'}</td>
  ...
  const getFilteredResponses = () => {
    // Responses are already filtered by SK user's barangay when loaded, so start with all responses
    let filtered = [...responses];

    // Apply search filter
    if (searchQuery) {
      filtered = filtered.filter(response => {
        // Search by anonymous identifier (for privacy), email, or location
        const anonymousId = getAnonymousName(response);
        return anonymousId.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    // For SK users, location filter is not needed since responses are already filtered by their barangay
    // But we can still apply it if user manually changes it (though it should be locked to their barangay)
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-3">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            {/* Overview Controls - export only (location filter hidden for SK users - they only see their barangay) */}
            {viewMode === 'overview' && (
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 overflow-x-auto pb-2 md:pb-0">
                    {/* Location filter removed for SK users - they only see their barangay */}
                    {skBarangay && (
                      <div className="text-sm text-gray-600">
                        Showing data for: <span className="font-medium text-gray-900">{skBarangay}</span>
                      </div>
                    )}
                  </div>
            ...
            {/* Analytics controls - location filter hidden for SK users */}
            {viewMode === 'analytics' && (
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 overflow-x-auto pb-2 md:pb-0">
                    ...
                    {skBarangay && (
                      <div className="text-sm text-gray-600">
                        Showing analytics for: <span className="font-medium text-gray-900">{skBarangay}</span>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center space-x-3 flex-shrink-0">
