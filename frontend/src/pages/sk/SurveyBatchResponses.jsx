...
const SurveyBatchResponses = ({
  ...
  // Helper function to anonymize youth names for privacy
  const getAnonymousName = (response) => {
    // Use response ID if available, otherwise generate a stable identifier
    // This ensures privacy while maintaining consistent identification
    const identifier = response.responseId || response.id;
    if (identifier) {
      return `Respondent #${identifier}`;
    }
    // Fallback: create a stable hash from response data if no ID exists
    // This ensures the same response always gets the same anonymous name
  ...
  const getFilteredResponses = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((response) => {
        // Search by anonymous identifier (e.g., "respondent #123"), email, or location
        // Not searching by actual name for privacy protection
        const anonymousName = getAnonymousName(response);
        return anonymousName.toLowerCase().includes(query) ||
          (response.email || '').toLowerCase().includes(query) ||
          (response.location || response.barangay || '').toLowerCase().includes(query);
      });
    }
    if (statusFilter !== 'all') {
      filtered = filtered.filter(response => {
            ...
            <div className="flex items-center space-x-3 min-w-max">
              ...
              <div className="flex-shrink-0">
                <SearchBar
                  ...
                  placeholder="Search by respondent #, email, or location..."
                  expandOnMobile={true}
                  showIndicator={true}
          ...
          {paginatedResponses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2 md:gap-3">
              {paginatedResponses.map((r, idx) => {
                ...
                // Anonymize name for privacy - use anonymous identifier instead of actual name
                const anonymousName = getAnonymousName(r);
                const age = r.age ?? '';
                ...
                return (
                  <div key={r.responseId || r.id || idx} className="bg-white rounded-lg md:rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition">
                    <div className="px-3 py-2 md:px-4 md:py-2.5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                      <div className="font-semibold text-gray-900 truncate mr-2 max-w-[70%] text-sm md:text-base">{anonymousName}</div>
