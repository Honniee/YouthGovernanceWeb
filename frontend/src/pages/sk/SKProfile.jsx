...
import { User, Mail, Shield, IdCard, Phone, Save, Edit2, ImagePlus, Trash2, CheckCircle2, Calendar, MapPin, GraduationCap, Building } from 'lucide-react';
...
const SKProfile = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setProfile({
        ...
        barangay_name: user.barangayName || user.barangay_name || '',
        term_name: user.termName || user.term_name || '',
        ...
        school_or_company: user.schoolOrCompany || user.school_or_company || ''
      });
  ...
  const positionDisplay = useMemo(() => {
    const pos = (profile.position || '').toString();
    return pos || '—';
  }, [profile.position]);

  const roleDisplay = useMemo(() => {
    const r = (profile.role_name || '').toString().toLowerCase();
    if (r === 'sk') return 'SK Official';
  ...
  const isPhoneNumber = (val) => /^(\+63|0)?[0-9]{10}$/.test(String(val || '').replace(/\s/g, ''));
  ...
  const dirtyPersonal = useMemo(() => {
    ...
    return (
      ...
      profile.contact_number !== orig.contact_number ||
      profile.school_or_company !== orig.school_or_company
    );
  }, [profile.first_name, profile.last_name, profile.middle_name, profile.suffix, profile.contact_number, profile.school_or_company]);

  const validPersonal = useMemo(() => {
    return String(profile.first_name).trim().length > 0 && 
           String(profile.last_name).trim().length > 0 &&
           (!profile.contact_number || isPhoneNumber(profile.contact_number));
  }, [profile.first_name, profile.last_name, profile.contact_number]);
  ...
  }, [profile.first_name, profile.middle_name, profile.last_name, profile.suffix, profile.personal_email, profile.contact_number, profile.school_or_company]);
  ...
  const settingsErrors = useMemo(() => {
    ...
    const cn = String(profile.contact_number || '').trim();
    if (cn && !isPhoneNumber(cn)) errors.contact_number = 'Enter a valid contact number';
    if (!String(profile.school_or_company || '').trim()) errors.school_or_company = 'School/Company is required';
    return errors;
  }, [profile.first_name, profile.last_name, profile.personal_email, profile.contact_number, profile.school_or_company]);

  const validSettings = Object.keys(settingsErrors).length === 0;

  // Profile completion logic for SK users
  const profileCompletion = useMemo(() => {
    const items = [
      ...
      },
      {
        ...
        weight: 15,
        completed: !!(profile.contact_number && profile.school_or_company),
        icon: Phone,
        color: 'purple',
        action: () => setActiveTab('settings')
      },
  ...
  }, [profile.email, profile.first_name, profile.last_name, profile.profile_picture, profile.personal_email, profile.contact_number, profile.school_or_company, profile.middle_name, profile.suffix]);
    ...
    try {
      ...
      if (res.success && res.user) {
        ...
        originalProfileRef.current = {
          ...
          contact_number: res.user.contactNumber ?? updates.contact_number,
          school_or_company: res.user.schoolOrCompany ?? updates.school_or_company
        };
        ...
        setProfile((prev) => ({
          ...
          contact_number: res.user.contactNumber ?? prev.contact_number,
          school_or_company: res.user.schoolOrCompany ?? prev.school_or_company,
  ...
  const handleCancelSettings = () => {
    ...
    setProfile(prev => ({
      ...
      contact_number: orig.contact_number || '',
      school_or_company: orig.school_or_company || ''
    }));
  };
  ...
  return (
    <div className="space-y-6">
      <HeaderMainContent 
        title="SK Profile" 
        description="Manage your personal information and account details as an SK Official."
      />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <TabContainer activeTab={activeTab} onTabChange={setActiveTab} variant="underline" size="md">
            ...
            <Tab id="settings" label="Settings" shortLabel="Settings" color="green" />
          </TabContainer>
          <form className="p-6 border-t border-gray-100" onSubmit={onSubmitSettings}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                  ...
                  <div className="md:col-span-9">
                    <div className="space-y-1 max-w-5xl">
                      ...
                      <InfoRow label="Position" value={positionDisplay} />
                      <InfoRow label="Barangay" value={profile.barangay_name || '—'} />
                      <InfoRow label="Term" value={profile.term_name || '—'} />
                      ...
                      <InfoRow label="School/Company" value={profile.school_or_company || '—'} />
            ...
            {activeTab === 'settings' && (
              <div className="space-y-6">
                {/* Header */}
                <div className="text-sm text-gray-600">Update your SK profile information</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-3 items-start">
                  ...
                  <div className="md:col-span-3 flex flex-col items-center gap-4">
                    <div className="flex flex-col items-center gap-3">
                      <Avatar name={fullName || profile.email} src={profile.profile_picture || avatarPreview} size={116} version={profile.updated_at} />
                      <button type="button" onClick={() => fileInputRef.current?.click()} className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-green-600 text-white text-sm shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-400">
                  ...
                  <div className="md:col-span-9">
                    <div className="space-y-4">
                      <EditableRow label="Last Name">
                        <input value={profile.last_name || ''} onChange={(e) => setProfile({ ...profile, last_name: e.target.value })} className={`w-full px-3 py-2 rounded-md border ${settingsErrors.last_name ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-200 focus:ring-green-500 focus:border-green-500'} bg-white`} />
                      </EditableRow>
                      <EditableRow label="First Name">
                        <input value={profile.first_name || ''} onChange={(e) => setProfile({ ...profile, first_name: e.target.value })} className={`w-full px-3 py-2 rounded-md border ${settingsErrors.first_name ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-200 focus:ring-green-500 focus:border-green-500'} bg-white`} />
                      </EditableRow>
                      <EditableRow label="Middle Name">
                        <input value={profile.middle_name || ''} onChange={(e) => setProfile({ ...profile, middle_name: e.target.value })} className="w-full px-3 py-2 rounded-md border border-gray-200 bg-white focus:ring-2 focus:ring-green-500 focus:border-green-500" />
                      </EditableRow>
                      <EditableRow label="Suffix">
                        <input value={profile.suffix || ''} onChange={(e) => setProfile({ ...profile, suffix: e.target.value })} className="w-full px-3 py-2 rounded-md border border-gray-200 bg-white focus:ring-2 focus:ring-green-500 focus:border-green-500" />
                      </EditableRow>
                      <EditableRow label="Personal Email">
                        <input value={profile.personal_email || ''} onChange={(e) => setProfile({ ...profile, personal_email: e.target.value })} className={`w-full px-3 py-2 rounded-md border ${settingsErrors.personal_email ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-200 focus:ring-green-500 focus:border-green-500'} bg-white`} />
                      </EditableRow>
                      <EditableRow label="Contact Number">
                        <input value={profile.contact_number || ''} onChange={(e) => setProfile({ ...profile, contact_number: e.target.value })} className={`w-full px-3 py-2 rounded-md border ${settingsErrors.contact_number ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-200 focus:ring-green-500 focus:border-green-500'} bg-white`} placeholder="09XXXXXXXXX" />
                      </EditableRow>
                      <EditableRow label="School/Company">
                        <input value={profile.school_or_company || ''} onChange={(e) => setProfile({ ...profile, school_or_company: e.target.value })} className={`w-full px-3 py-2 rounded-md border ${settingsErrors.school_or_company ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-200 focus:ring-green-500 focus:border-green-500'} bg-white`} />
                      </EditableRow>
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          ...
          {profileCompletion.percentage < 100 && (
            <div className="xl:hidden fixed bottom-6 right-6 z-50">
              <button
                onClick={() => setActiveTab('settings')}
                className="w-14 h-14 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-full shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-200 flex items-center justify-center"
              >
            ...
            <div className="px-6 pb-6">
              ...
              {profileCompletion.percentage === 100 && (
                <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 animate-in slide-in-from-bottom-4 duration-500">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-emerald-800">SK Profile Complete!</h4>
                      <p className="text-xs text-emerald-600">Excellent work! Your SK profile is fully optimized.</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {profileCompletion.percentage < 100 ? (
                <div className="mt-4 space-y-2">
                  <div className="p-3 rounded-lg bg-green-50/50 border border-green-200/50">
                    <p className="text-xs text-green-600 text-center">
                      💡 Complete your SK profile to unlock better features and improve your experience
                    </p>
                  </div>
                  
                  {/* Benefits list */}
                  <div className="p-3 rounded-lg bg-gradient-to-r from-emerald-50/50 to-green-50/50 border border-emerald-200/50">
                    <h5 className="text-xs font-semibold text-emerald-700 mb-2">Benefits of completing:</h5>
                ...
                <div className="mt-4 p-3 rounded-lg bg-gradient-to-r from-emerald-50/50 to-green-50/50 border border-emerald-200/50">
                  ...
                  <p className="text-xs text-emerald-600">You now have access to all SK features and enhanced security.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-[calc(100vw-24px)] sm:max-w-xl h-[90vh] sm:h-auto max-h-[90vh] overflow-y-auto flex flex-col p-3 sm:p-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3 sm:justify-end">
                ...
                <button type="button" onClick={handleCropSave} className="px-3 py-1.5 rounded-lg bg-green-600 text-white">{isUploading ? `Saving ${uploadProgress}%` : 'Use Photo'}</button>
              </div>
            </div>
          </div>
        </div>
