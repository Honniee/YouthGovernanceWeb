...
const SurveyBatch = () => {
  ...
  // Check if a batch has responses from SK user's barangay (memoized)
  const batchHasBarangayResponses = useCallback(async (batchId) => {
    if (!skBarangay || !batchId) return false;
    try {
      // Make a lightweight check (limit=1) to see if any responses exist from this barangay
      const response = await surveyBatchesService.getBatchResponses(batchId, {
        limit: 1,
        barangay: skBarangay
      });
  ...
  // Get barangay-filtered statistics for a batch
  const getBarangayFilteredStats = useCallback(async (batchId) => {
    if (!skBarangay || !batchId) {
      return { total: 0, validated: 0, pending: 0, rejected: 0 };
    }
    
    try {
      // Fetch all responses for this batch filtered by barangay
      const response = await surveyBatchesService.getBatchResponses(batchId, {
      ...
      if (response?.success) {
        ...
        if (Array.isArray(response.data?.data)) {
          ...
          responses = response.data.data.data;
        }
        
        // Calculate statistics from filtered responses
        const total = responses.length;
        const validated = responses.filter(r => {
          const status = (r.status || r.validationStatus || r.validation_status || '').toString().toLowerCase();
  ...
  const loadBatchData = async (...args) => {
    ...
    try {
      // Step 1: Fetch ALL batches first (all statuses: active, closed, draft) - no filters at API level
      const params = {
        page: 1, // Always fetch from page 1 to get all batches
        limit: 1000, // Fetch a large number to get all batches
        sortBy,
        sortOrder,
        search: searchQuery || undefined,
        status: undefined, // Fetch ALL statuses (active, closed, draft)
        includeStats: true, // Always include statistics for proper display
        // No barangay filter - we'll filter on frontend
      };
      ...
      if (response.success) {
        ...
        if (searchQuery) {
          ...
          batchData = batchData.filter(batch => 
            (batch.batchName || batch.batch_name || '').toLowerCase().includes(query) ||
        ...
        batchData.sort((a, b) => {
          ...
          switch (sortBy) {
            case 'batch_name':
              aValue = (a.batchName || a.batch_name || '').toLowerCase();
              bValue = (b.batchName || b.batch_name || '').toLowerCase();
              break;
            case 'start_date':
              aValue = new Date(a.startDate || a.start_date || 0);
              bValue = new Date(b.startDate || b.start_date || 0);
              break;
            case 'end_date':
              aValue = new Date(a.endDate || a.end_date || 0);
              bValue = new Date(b.endDate || b.end_date || 0);
              break;
            case 'created_at':
            default:
        ...
        // Step 5: Update statistics for each batch based on barangay-filtered responses
        if (skBarangay) {
          const batchesWithBarangayStats = await Promise.all(
            batchData.map(async (batch) => {
              const batchId = batch.batchId || batch.batch_id || batch.id;
              const stats = await getBarangayFilteredStats(batchId);
              
              // Update batch statistics with barangay-filtered data
              return {
                ...batch,
                statisticsTotalResponses: stats.total,
                statisticsValidatedResponses: stats.validated,
                statisticsPendingResponses: stats.pending,
                statisticsRejectedResponses: stats.rejected,
                // Also update snake_case versions if they exist
                total_responses: stats.total,
                validated_responses: stats.validated,
                pending_responses: stats.pending,
                rejected_responses: stats.rejected,
              };
            })
          );
          batchData = batchesWithBarangayStats;
        }
        
        // Step 6: Apply pagination to filtered results
        const startIndex = ((customPage || currentPage) - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const paginatedBatches = batchData.slice(startIndex, endIndex);
  ...
  const loadBatchStats = useCallback(async (opts = { silent: false }) => {
    try {
      ...
      const response = await surveyBatchesService.getSurveyBatches({
        page: 1,
        limit: 1000, // Get all batches to calculate accurate stats
        includeStats: true
      });
      ...
      if (response.success) {
        ...
        // Filter batches that have responses from SK user's barangay
        if (skBarangay) {
          const filteredBatches = [];
          for (const batch of allBatches) {
            const hasResponses = await batchHasBarangayResponses(batch.batchId || batch.batch_id || batch.id);
            if (hasResponses) {
        ...
        const mappedStats = {
          ...
          active: allBatches.filter(b => (b.status || '').toLowerCase() === 'active').length,
  ...
  // Note: loadBatchData is already a function, we just need to reference it in dependencies
  const memoizedLoadBatchData = useCallback(() => {
    loadBatchData();
  }, [
    currentPage, 
    itemsPerPage, 
    ...
    statusFilter,
    skBarangay,
    batchHasBarangayResponses,
    getBarangayFilteredStats
  ]);
  ...
  return (
    <div className="space-y-5">
      ...
      <ActiveSurveyBanner
        ...
        onViewSurvey={(survey) => {
          navigate(`/sk/survey/batches/batch-report?batchId=${survey.batchId}`);
        }}
        variant="report"
      />
                      ...
                      batches.map((item) => {
                        ...
                        return (
                          <div key={item.batchId} className="group relative h-full">
                            ...
                            {/* Card */}
                              <div className="relative rounded-2xl overflow-hidden shadow-sm transition-all duration-200 group-hover:shadow-lg h-full flex flex-col cursor-pointer bg-white ring-1 ring-gray-200" onClick={() => navigate(`/sk/survey/batches/batch-report?batchId=${item.batchId}`)}>
                              {/* Image header */}
