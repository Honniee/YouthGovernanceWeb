...
const SKTermReport = () => {
  ...
  // Check if user is SK user (view-only access)
  const isSKUser = user?.userType === 'sk_official' || user?.role === 'sk_official';
  ...
  // SK users can only view federation tab
  const [tabLoading, setTabLoading] = useState(false);
  const initialTab = isSKUser ? 'federation' : 'overview';
  const { activeTab: viewMode, setActiveTab: setViewMode } = useTabState(initialTab, async (tabId) => {
    setTabLoading(true);
    try {
      setCurrentPage(1);
...
${bodyRows}
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredBarangays.length === 0 ? (
                              <tr>
                                <td colSpan={Object.keys(POSITION_LIMITS).length + 3} className="px-6 py-10 text-center text-gray-500">
  ...
  {/* Edit Federation Modal - Removed for staff (view-only) */}
  {false && showEditFederation && (
