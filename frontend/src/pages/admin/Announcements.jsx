...
const Announcements = () => {
  ...
  // Realtime refresh on announcement changes
  useRealtime('announcement:changed', () => {
    // Silent refresh to avoid loading flash; keep current page
    fetchAnnouncementsData({ silent: true });
    refreshStatisticsSilent();
  });
  ...
  const handleActionClick = async (action, item) => {
    ...
    try {
      ...
      switch (action) {
        case 'view':
          navigate(`/admin/announcements/${item.announcement_id || item.id}`);
          break;
        case 'edit':
          navigate(`/admin/announcements/${item.announcement_id || item.id}/edit`);
          break;
        case 'publish':
          // Confirm publish
          {
  ...
  // Silent statistics refresh (no loading flags)
  const refreshStatisticsSilent = async () => {
    try {
      const statsResponse = await getAnnouncementStatistics();
      if (statsResponse.success && statsResponse.data) {
        setStatistics({
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            <button
              ...
              onClick={() => navigate('/admin/announcements/featured')}
            >
              View Featured
            </button>
          </div>
                ...
                return (
                  <div key={key} className="group relative h-full">
                    ...
                    <div 
                      ...
                      onClick={() => navigate(`/admin/announcements/${a.announcement_id || a.id}`)}
                    >
                      {/* Selection Checkbox */}
