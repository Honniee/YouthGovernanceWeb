...
const SurveyBatchSegmentation = ({ batchId, batchName }) => {
  ...
  useEffect(() => {
    const loadBarangays = async () => {
      try {
        ...
        logger.error('Failed to load barangays', err);
      }
    };
    loadBarangays();
  }, []);
  ...
  const fetchSegmentationData = async () => {
    ...
    try {
      ...
      logger.error('Error fetching segmentation data', err, { filters: { scope, selectedBarangay, batchId } });
  ...
  const fetchRecommendations = async (segmentId) => {
    ...
    try {
      ...
      logger.debug('Segmentation recommendations response', response);
      ...
      logger.debug('Segmentation recommendations extracted', { count: Array.isArray(recommendationsData) ? recommendationsData.length : 0 });
      setRecommendations(Array.isArray(recommendationsData) ? recommendationsData : []);
    } catch (err) {
      logger.error('Error fetching recommendations', err, { segmentId });
      setRecommendations([]);
    } finally {
  ...
  const handleRunClustering = async () => {
    ...
    try {
      ...
      logger.error('Error running clustering', err, {
        scope,
        selectedBarangay,
        batchId
      });
