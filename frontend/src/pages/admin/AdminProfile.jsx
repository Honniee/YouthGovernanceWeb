...
const AdminProfile = () => {
  ...
  useEffect(() => {
    if (user) {
      logger.debug('AdminProfile received user context', { user });
      setProfile({
  ...
  const handleResendVerification = () => {
    // Demo only
    logger.info('AdminProfile resend verification requested');
  };
  ...
  const handleSaveSettings = async () => {
    logger.debug('AdminProfile save triggered');
    ...
    try {
      ...
      if (res.success && res.user) {
        ...
        logger.warn('AdminProfile update failed', { message: res.message });
        showErrorToast && showErrorToast('Update failed', res.message || 'Please try again');
      }
    } catch (e) {
      logger.error('Failed to save admin profile settings', e);
